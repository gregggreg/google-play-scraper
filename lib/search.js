...
function getClp (html) {
  ...
  if (!match) match = html.match(/\?clp\\u003d(.*?)"/);
  if (!match) {
    var err = new Error("Unable to parse Google Play results");
    err.code = 'ENOTFOUND';
    throw err;
  }
  return match && match[1].replace(/%3D/g, '=').replace('\\u0026', '&amp;').replace('\\u003d', '=');
}
...
function getNextToken (html) {
  ...
  var s = html.match(/\\x22-p6(.*?):S:(.*?)\\x22/g);
  if (s) {
  ...
  return s[1].replace(/\\\\u003d/g, '=').replace(/\\x22/g, '');
}
...
=======
    form: {
      num: 48, // confirm if always 48 works
      start: savedApps.length,
      pagTok: nextToken,
      clp,
      pagtt: 3,
      hl: opts.lang,
    ...
    followAllRedirects: true
>>>>>>> Be brittle. Throw error if parsing fails in search rather than failing silently. Fix pagination issue.
  }, opts.requestOptions);
...
function initialRequest (opts) {
  ...
  opts.retryAttemptNumber = (opts.retryAttemptNumber || 0) + 1;
  ...
  return request(Object.assign({ url }, opts.requestOptions), opts.throttle)
    ...
    .catch((err) => {
      ...
      const retry = opts.requestOptions.retryStrategy;
      if (retry && opts.retryAttemptNumber <= (opts.requestOptions.maxAttempts || 0)) {
        if (retry(err, null, null, null)) {
          debug("RETRYING");
